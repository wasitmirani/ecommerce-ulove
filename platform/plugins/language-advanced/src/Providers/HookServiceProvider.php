...
class HookServiceProvider extends ServiceProvider
{
    ...
    public function checkCurrentLanguage($languages)
    {
        ...
        foreach ($languages as $language) {
            if (
                ($request->input('ref_lang') && $language->lang_code == $request->input('ref_lang')) ||
    ...
    public function getDataLanguageBeforeShow($data, $model)
    {
        ...
        $lang = DB::table($translationTable)->where($table . '_id', $data->value('id'));
        if ($lang->first()) {
            ...
            $lang = $lang->where($translationTable . '.lang_code', Language::getCurrentAdminLocaleCode());
            if ($lang->first()) {
                $data = $data->where($translationTable . '.lang_code', Language::getCurrentAdminLocaleCode());
            } else {
                $data = $data->where($translationTable . '.lang_code', Language::getDefaultLocaleCode());
            }
        }
