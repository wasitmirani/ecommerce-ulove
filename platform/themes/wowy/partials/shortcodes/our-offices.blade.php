            ...
            @foreach(json_decode(theme_option('contact_info_boxes'), true) as $item)
                @if (count($item) == 4)
                    <div class="col-md-4 mb-4 mb-md-0">
                        <h4 class="mb-15 text-muted">{!! clean($item[0]['value']) !!}</h4>
                        {!! clean($item[1]['value']) !!}<br>
                        <abbr title="{{ __('Phone') }}">{{ __('Phone') }}:</abbr> {!! clean($item[2]['value']) !!}<br>
                        <abbr title="{{ __('Email') }}">{{ __('Email') }}: </abbr>{!! clean($item[3]['value']) !!}<br>
